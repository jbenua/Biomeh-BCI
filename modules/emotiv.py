...
def get_level(data, bits):
    """
    Returns sensor level value from data
    using sensor bit mask in micro volts (uV).
    """
    level = 0
...
def get_linux_setup():
    ...
    for filename in os.listdir("/sys/class/hidraw"):
        real_path = check_output(
            ["realpath", "/sys/class/hidraw/" + filename]).decode()
    ...
    for input in raw_inputs:
        try:
            ...
            if "Emotiv Systems" in manufacturer:
                ...
                # The dev headset might use the first device,
                # or maybe if more than one are connected they might.
                hidraw_id += 1
...
class EmotivPacket:
    ...
    def __init__(self, data, sensors, model):
        ...
        global g_battery
        self.raw_data = data
        self.counter = data[0]  # ord(data[0])
    ...
    def handle_quality(self, sensors):
        """
        Sets the quality value for the sensor
        from the quality bits in the packet data.
        ...
        sensor = self.raw_data[0]  # ord(self.raw_data[0])
